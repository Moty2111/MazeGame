...
export default function Game({ level, onComplete, onBack, settings }) {
  ...
  useEffect(() => {
    const newMaze = generateMaze(level.rows, level.cols);
    setMaze(newMaze);
  ...
  const doMove = useCallback((direction) => {
    if (isWinRef.current || !mazeRef.current) return;

    const newPos = tryMove(playerPosRef.current, direction, mazeRef.current);
    if (newPos === playerPosRef.current) return;

    setPlayerPos(newPos);
    setMoves((m) => m + 1);

    const goalRow = level.rows - 1;
    const goalCol = level.cols - 1;

    if (newPos.row === goalRow && newPos.col === goalCol) {
      setIsWin(true);
      Animated.sequence([
  ...
  }, [settings.controlMode, doMove]);

  const handleSwipeOrButton = useCallback((direction) => {
    doMove(direction);
  }, [doMove]);

  const handleNext = () => {
    onComplete(level.id, moves, time);
  };
  ...
  const mazeSize = width - 48;
  const cellSize = Math.floor(mazeSize / Math.max(level.cols, level.rows));
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.topInfo}>
          ...
          <Text style={styles.levelSub}>
            Уровень {level.id + 1} • {level.rows}×{level.cols}
          </Text>
        </View>
        ...
        <View style={styles.completePanel}>
          ...
          <TouchableOpacity style={styles.nextBtn} onPress={handleNext} activeOpacity={0.8}>
            <Text style={styles.nextBtnText}>
              {level.id < 4 ? '→ Следующий уровень' : '🎊 Финальный уровень!'}
            </Text>
          </TouchableOpacity>
        </View>
      )}
        ...
        <View style={styles.controlsContainer}>
          <View style={styles.dpad}>
            <View style={styles.dpadRow}>
              <TouchableOpacity style={[styles.dpadBtn, styles.dpadPlaceholder]} disabled />
              <TouchableOpacity style={styles.dpadBtn} onPress={() => handleSwipeOrButton('UP')} activeOpacity={0.6}>
                <Text style={styles.dpadBtnText}>↑</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.dpadBtn, styles.dpadPlaceholder]} disabled />
            </View>
            <View style={styles.dpadRow}>
              <TouchableOpacity style={styles.dpadBtn} onPress={() => handleSwipeOrButton('LEFT')} activeOpacity={0.6}>
                <Text style={styles.dpadBtnText}>←</Text>
              </TouchableOpacity>
              <View style={[styles.dpadBtn, styles.dpadCenter]}>
                <Text style={styles.dpadCenterText}>🐱</Text>
              </View>
              <TouchableOpacity style={styles.dpadBtn} onPress={() => handleSwipeOrButton('RIGHT')} activeOpacity={0.6}>
                <Text style={styles.dpadBtnText}>→</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.dpadRow}>
              <TouchableOpacity style={[styles.dpadBtn, styles.dpadPlaceholder]} disabled />
              <TouchableOpacity style={styles.dpadBtn} onPress={() => handleSwipeOrButton('DOWN')} activeOpacity={0.6}>
                <Text style={styles.dpadBtnText}>↓</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.dpadBtn, styles.dpadPlaceholder]} disabled />
            </View>
          ...
          <View style={styles.swipeHint}>
            <Text style={styles.swipeHintText}>👆 Проведи по лабиринту или нажми стрелки</Text>
          </View>
        </View>
      )}
    </View>
