...
const CONFETTI_COLORS = [
  '#D8B4FE', '#86EFAC', '#FDE68A', '#FCA5A5',
  '#93C5FD', '#FDBA74', '#C4B5D4', '#FBBF24',
];

function ConfettiPiece({ delay, duration, color, side }) {
  ...
  const translateX = anim.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [side * 50, side * 30, side * 80],
  });

  const rotate = anim.interpolate({
    inputRange: [0, 1],
    outputRange: ['0deg', `${side * 360}deg`],
  });
...
export default function Finish({ onRestart, levelResults }) {
  ...
  const totalMoves = levelResults.reduce((sum, r) => sum + r.moves, 0);
  const totalTime = levelResults.reduce((sum, r) => sum + r.time, 0);
  ...
  const catSize = bounceAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, 1],
  });
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        ...
        <Animated.View style={{ transform: [{ scale: catSize }] }}>
        ...
        <View style={styles.statsCard}>
          ...
          <Text style={styles.statsText}>
            Уровней пройдено: {levelResults.length} из 5
          </Text>
          <Text style={styles.statsText}>
